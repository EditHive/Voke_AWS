...
export function useGeminiLive(): UseGeminiLiveReturn {
  ...
  const manualDisconnectRef = useRef(false); // Track if close was user-initiated
  ...
  const disconnect = useCallback(() => {
    // Mark this as a user-initiated disconnect so onclose doesn't treat it as an error
  ...
  const connect = useCallback(async (context?: string) => {
    ...
    try {
      ...
      if (!apiKey) {
        ...
        setErrorDetails('API key missing in .env');
        return;
      }
      console.log('Using API Key ending in:', apiKey.slice(-5));

      // Use the v1alpha API version which is required for some Live API
      // features in web environments and avoids known websocket path issues.
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onclose: (event: any) => {
            ...
            // If we closed the session from the UI, we have already
            // cleaned everything up in disconnect(). Just reset the flag.
            if (manualDisconnectRef.current) {
              manualDisconnectRef.current = false;
              return;
            }

            // Unexpected close from the server  treat as an error
            cleanupAudioContexts();
            setStatus(LiveStatus.ERROR);

            const code = event.code || 'Unknown';
            const reason = event.reason || 'No reason provided';
            setErrorDetails(`Session closed: ${code} ${reason}`);
          ...
          onerror: (err) => {
            console.error('Session error full object:', err);
